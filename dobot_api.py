...
class DobotClaw():
    ...
    def _init_(self):     
        #夹爪初始化
        self.s_obj = serial.Serial("/dev/ttyUSB0", baudrate=115200, bytesize=8, parity='N', stopbits=1, timeout=1)
        test_init = "01 06 01 00 00 01 49 F6"
        self.s_obj.write(bytes().fromhex(test_init)) #pyserial串口模块里write的输入参数必须是bytes（二进制）格式
        data = self.s_obj.read(800*80)
        #print(data)
    ...
    #设定夹爪开合程度
        poslist = "01 06 01 03 "
        data = '{:04x}'.format(pos)   #将位置量转为四位十六进制
        poslist +=  data[0:2] + ' '+ data[2:]  #加上设定的位置
        crc16 = self.mudbus_crc_16(poslist)
        poslist += ' '+ crc16[2:4] + ' ' + crc16[4:]  #加上校验码
        self.s_obj.write(bytes().fromhex(poslist))
        # print(data)
        print('pos=',poslist)
    ...
    #设定夹爪开合速度
        spelist = "01 06 01 04 "
        data = '{:04x}'.format(spe)
        spelist += data[0:2] + ' '+ data[2:]  #加上设定的速度
        crc16 = self.mudbus_crc_16(spelist)
        spelist += ' '+ crc16[2:4] + ' ' + crc16[4:]  #加上校验码
        self.s_obj.write(bytes().fromhex(spelist))
        print('speed=',spelist)
    ...
    #设定夹爪开合力度
        powlist = "01 06 01 01 "
        data = '{:04x}'.format(pow)
        powlist += data[0:2] + ' '+ data[2:]  #加上设定的力度
        crc16 = self.mudbus_crc_16(powlist)
        powlist += ' '+ crc16[2:4] + ' ' + crc16[4:]  #加上校验码
        self.s_obj.write(bytes().fromhex(powlist))
        print('power=',powlist)
        
    def readstate(self):
    #读取夹爪开合状态
        state = "01 03 02 01 00 01 D4 72"
        r_obj = serial.Serial("/dev/ttyUSB0", baudrate=115200, bytesize=8, parity='N', stopbits=1, timeout=1)
        r_obj.write(bytes().fromhex(state))
        data = r_obj.read(150*80)
        if data[4] == 1:
             print("夹爪停止运动，且夹爪未检测到夹到物体")
        elif data[4]==2:
             print("夹爪停止运动，且夹爪检测到夹到物体")
        print(data)

    def readposition(self):
    #读取夹爪开合程度
        state = "01 03 02 02 00 01 24 72"
        r_obj = serial.Serial("/dev/ttyUSB0", baudrate=115200, bytesize=8, parity='N', stopbits=1, timeout=1)
        r_obj.write(bytes().fromhex(state))
        data = r_obj.read(150*80)
        pos = int(data[3])*256 + int(data[4])
        print('夹爪开合程度位于',pos)
